...
const TopSidebar: React.FC<TopSidebarProps> = ({ user, onLogout, onNotificationClick, onNavigate, onToggleMobileNav, onToggleMobileActivity, unreadCount = 0 }) => {
  ...
  const performSearch = (query: string) => {
      ...
      const results = {
          articles: mockArticles.filter(a => a.title.toLowerCase().includes(lowerQuery)),
        ...
        <form onSubmit={(e) => { e.preventDefault(); performSearch(searchQuery); setIsSearchFocused(true); }} className="relative w-full max-w-lg flex items-center gap-2" ref={searchRef}>
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-[--color-surface-primary] border border-transparent focus:bg-[--color-surface-secondary] focus:border-[--color-accent-400] focus:ring-0 focus:outline-none placeholder:text-[--color-text-subtle] text-[--color-text-primary] rounded-full py-2 pl-11 pr-4 transition-all duration-300"
              aria-label={t('search')}
              value={searchQuery}
          ...
          <button type="submit" className="flex items-center justify-center p-2 rounded-full bg-[--color-accent-500] text-white hover:bg-[--color-accent-600] transition-colors" aria-label="Submit Search">
